 ...
 *
 * Parameters:
 *  @param[in]  <type*>     diag: a vector of 'n' diagonal elements.
 *  @param[in]  <type*>     offd: a vector of 'n-1' off-diagonal elements.
 *  @param[in]  int            n: The order of the matrix.
 *  @param[in]  <type>         u: The sigma test point.
 *  @param[out] int            r: Number of eigenvalues strictly less than u.
 *
 * This code is copied from the PLASMA library.
 *
...
int64_t laneg2(scalar_t *diag, scalar_t *offd, int64_t n, scalar_t u)
{
    ...
    for (i=1; i<n; i++) {
        ...
        if (w > PHI) {  // If the largest magnitude > big,
            ...
            Pm1_0 *= s; // apply it.
            Pm1_1 *= s;
        } else if (w < UPSILON) {   // if largest magnitude < tiny, 
