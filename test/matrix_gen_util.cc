...
#else
    lapack::Error e ("unknown platform", "creation");
#endif
...
extern "C"
void matrix_assert_warn( bool condition, const char* msg, ... );
extern "C"
void matrix_assert( bool condition, const char* msg, ... );
...
"\n"
"  --matrix name    name of test matrix type from matrix_generate, default 'rand',\n"
"                   or 'rand_dominant' if SPD required (e.g., for posv).\n"
"  --cond   kA      where applicable, condition number for test matrix, default sqrt( 1/eps ); see matrix_generate_matrix.\n"
"  --condD  kD      where applicable, condition number for scaling test matrix, default 1; see matrix_generate_matrix.\n"
"\n"
"                   * default values\n";


// constructor fills in default values
matrix_opts::matrix_opts( matrix_opts_t flag )
{
    int nt = 1;  // TODO: mxagma_get_lapack_numthreads();
    ...
    this->nrhs     = 1;
    this->nqueue   = 1;
    this->ngpu     = 1; //  TODO: mxagma_num_gpus();
    this->nsub     = 1;
    this->niter    = 1;
    this->nthread  = 1;
    this->offset   = 0;
    ...
    this->uplo      = lapack::Uplo::Lower;      // potrf, etc.
    this->transA    = lapack::Op::NoTrans;      // gemm, etc.
    this->transB    = lapack::Op::NoTrans;      // gemm
    this->side      = lapack::Side::Left;       // trsm, etc.
    this->diag      = lapack::Diag::NonUnit;    // trsm, etc.
    this->jobz      = lapack::Job::NoVec;       // heev:  no eigen vectors
    this->jobvr     = lapack::Job::NoVec;       // geev:  no right eigen vectors
    this->jobvl     = lapack::Job::NoVec;       // geev:  no left  eigen vectors
...
void matrix_opts::parse_opts( int argc, char** argv )
{
    printf( usage_short, argv[0] );
    
    //int64_t ndevices;
    //int64_t devices[ MAXGPUS ];
    ...
    for( int i = 1; i < argc; ++i ) {
              ...
              strcmp("--range", argv[i]) == 0) && i+1 < argc )
        {
            ...
            matrix_assert( valid, "error: '%s %s' is not valid, expected (m|m_start:m_end:m_step)[,(n|n_start:n_end:n_step)[,(k|k_start:k_end:k_step)]]\n",
                          argv[i-1], argv[i] );
            // if all zero steps, just give start point
            if ( m_step == 0 && n_step == 0 && k_step == 0 ) {
                matrix_assert( this->ntest < MAX_NTEST, "error: %s %s exceeded maximum number of tests (%d).\n",
        ...
        // ----- lapack options
        else if ( strcmp("-L",  argv[i]) == 0 ) { this->uplo = lapack::Uplo::Lower; }
        else if ( strcmp("-U",  argv[i]) == 0 ) { this->uplo = lapack::Uplo::Upper; }
        //else if ( strcmp("-F",  argv[i]) == 0 ) { this->uplo = MagmaFull; }
        else if ( strcmp("-F",  argv[i]) == 0 ) { this->uplo = lapack::Uplo::Lower; }  // TODO: fix this to work for 'full' matrix
        
        else if ( strcmp("-NN", argv[i]) == 0 ) { this->transA = lapack::Op::NoTrans;   this->transB = lapack::Op::NoTrans;   }
        else if ( strcmp("-NT", argv[i]) == 0 ) { this->transA = lapack::Op::NoTrans;   this->transB = lapack::Op::Trans;     }
        else if ( strcmp("-NC", argv[i]) == 0 ) { this->transA = lapack::Op::NoTrans;   this->transB = lapack::Op::ConjTrans; }
        else if ( strcmp("-TN", argv[i]) == 0 ) { this->transA = lapack::Op::Trans;     this->transB = lapack::Op::NoTrans;   }
        else if ( strcmp("-TT", argv[i]) == 0 ) { this->transA = lapack::Op::Trans;     this->transB = lapack::Op::Trans;     }
        else if ( strcmp("-TC", argv[i]) == 0 ) { this->transA = lapack::Op::Trans;     this->transB = lapack::Op::ConjTrans; }
        else if ( strcmp("-CN", argv[i]) == 0 ) { this->transA = lapack::Op::ConjTrans; this->transB = lapack::Op::NoTrans;   }
        else if ( strcmp("-CT", argv[i]) == 0 ) { this->transA = lapack::Op::ConjTrans; this->transB = lapack::Op::Trans;     }
        else if ( strcmp("-CC", argv[i]) == 0 ) { this->transA = lapack::Op::ConjTrans; this->transB = lapack::Op::ConjTrans; }
        else if ( strcmp("-T",  argv[i]) == 0 ) { this->transA = lapack::Op::Trans;     }
        else if ( strcmp("-C",  argv[i]) == 0 ) { this->transA = lapack::Op::ConjTrans; }
        
        else if ( strcmp("-SL", argv[i]) == 0 ) { this->side  = lapack::Side::Left;  }
        else if ( strcmp("-SR", argv[i]) == 0 ) { this->side  = lapack::Side::Right; }
        
        else if ( strcmp("-DN", argv[i]) == 0 ) { this->diag  = lapack::Diag::NonUnit; }
        else if ( strcmp("-DU", argv[i]) == 0 ) { this->diag  = lapack::Diag::Unit;    }
        
        else if ( strcmp("-JN", argv[i]) == 0 ) { this->jobz  = lapack::Job::NoVec; }
        else if ( strcmp("-JV", argv[i]) == 0 ) { this->jobz  = lapack::Job::Vec;   }
        
        else if ( strcmp("-LN", argv[i]) == 0 ) { this->jobvl = lapack::Job::NoVec; }
        else if ( strcmp("-LV", argv[i]) == 0 ) { this->jobvl = lapack::Job::Vec;   }
        
        else if ( strcmp("-RN", argv[i]) == 0 ) { this->jobvr = lapack::Job::NoVec; }
        else if ( strcmp("-RV", argv[i]) == 0 ) { this->jobvr = lapack::Job::Vec;   }
	...
	/*
                 ...
                 token = strtok( NULL, ", " ))
            {
                if ( *token == '\0' ) { } // ignore empty tokens 
                    ...
                    matrix_assert( false, "error: --svd-work '%s' is invalid\n", argv[i] );
                }
            }
            free( arg );
        }
    ...
    if ( this->ntest == 0 ) {
        int64_t n2 = this->default_nstart;  //1024 + 64;
    ...
    #ifdef HAVE_CUBLAS
    matrix_setdevice( this->device );
    #endif
    
    #ifdef HAVE_CUBLAS
    // handle for directly calling cublas
    this->handle = matrix_queue_get_cublas_handle( this->queue );
    #endif
}
// end parse_opts
    ...
    int64_t* il, int64_t* iu )
{
    ...
    *vu = NAN;
    if (this->fraction_lo != 0 || this->fraction_up != 1) {
        ...
        *range = lapack::Range::Index;
        *il = std::min( this->irange_lo, n );
        *iu = std::min( this->irange_up, n );
...
// -----------------------------------------------------------------------------
void matrix_opts::cleanup()
{
    //this->queue = NULL;
    //matrix_queue_destroy( this->queues2[0] );
    //matrix_queue_destroy( this->queues2[1] );
    //this->queues2[0] = NULL;
    //this->queues2[1] = NULL;
...
void matrix_flush_cache( size_t cache_size )
{
