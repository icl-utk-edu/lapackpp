...
static lapack_int LAPACKE_sprfs(
...
{
    return LAPACKE_csprfs( LAPACK_COL_MAJOR, uplo, n, nrhs, AP, AFP, ipiv, B, ldb, X, ldx, ferr, berr );
}
...
{
    return LAPACKE_zsprfs( LAPACK_COL_MAJOR, uplo, n, nrhs, AP, AFP, ipiv, B, ldb, X, ldx, ferr, berr );
}
...
void test_sprfs_work( Params& params, bool run )
{
    ...
    int64_t info_trf = lapack::sptrf( uplo, n, &AFP[0], &ipiv_tst[0] );
    if (info_trf != 0) {
        fprintf( stderr, "lapack::hptrf returned error %lld\n", (lld) info_trf );
    }

    // solve
    int64_t info_trs = lapack::sptrs( uplo, n, nrhs, &AFP[0], &ipiv_tst[0], &X_tst[0], ldb );
    if (info_trs != 0) {
    ...
    int64_t info_tst = lapack::sprfs( uplo, n, nrhs, &AP[0], &AFP[0], &ipiv_tst[0], &B[0], ldb, &X_tst[0], ldx, &ferr_tst[0], &berr_tst[0] );
    time = omp_get_wtime() - time;
    if (info_tst != 0) {
        fprintf( stderr, "lapack::sprfs returned error %lld\n", (lld) info_tst );
    }

    params.time.value() = time;
    // double gflop = lapack::Gflop< scalar_t >::sprfs( n, nrhs );
...
void test_sprfs( Params& params, bool run )
{
    switch (params.datatype.value()) {
        ...
        case libtest::DataType::Double:
            test_sprfs_work< double >( params, run );
            break;

        case libtest::DataType::SingleComplex:
            test_sprfs_work< std::complex<float> >( params, run );
            break;
