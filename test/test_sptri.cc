...
static lapack_int LAPACKE_sptri(
...
{
    return LAPACKE_csptri( LAPACK_COL_MAJOR, uplo, n, AP, ipiv );
}

static lapack_int LAPACKE_sptri(
    char uplo, lapack_int n, std::complex<double>* AP, lapack_int* ipiv )
{
    return LAPACKE_zsptri( LAPACK_COL_MAJOR, uplo, n, AP, ipiv );
}

// -----------------------------------------------------------------------------
template< typename scalar_t >
void test_sptri_work( Params& params, bool run )
{
    ...
    int64_t info_trf = lapack::sptrf( uplo, n, &AP_tst[0], &ipiv_tst[0] );
    if (info_trf != 0) {
    ...
    int64_t info_tst = lapack::sptri( uplo, n, &AP_tst[0], &ipiv_tst[0] );
    time = omp_get_wtime() - time;
    if (info_tst != 0) {
        fprintf( stderr, "lapack::sptri returned error %lld\n", (lld) info_tst );
    }

    params.time.value() = time;
    // double gflop = lapack::Gflop< scalar_t >::sptri( n );
...
void test_sptri( Params& params, bool run )
{
    switch (params.datatype.value()) {
        ...
        case libtest::DataType::Double:
            test_sptri_work< double >( params, run );
            break;

        case libtest::DataType::SingleComplex:
            test_sptri_work< std::complex<float> >( params, run );
            break;
