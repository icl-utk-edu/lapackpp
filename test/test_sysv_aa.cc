...
{
    return LAPACKE_csysv_aa( LAPACK_COL_MAJOR, uplo, n, nrhs, A, lda, ipiv, B, ldb );
}
...
{
    return LAPACKE_zsysv_aa( LAPACK_COL_MAJOR, uplo, n, nrhs, A, lda, ipiv, B, ldb );
}

// -----------------------------------------------------------------------------
template< typename scalar_t >
void test_sysv_aa_work( Params& params, bool run )
{
    ...
    int64_t info_tst = lapack::sysv_aa( uplo, n, nrhs, &A_tst[0], lda, &ipiv_tst[0], &B_tst[0], ldb );
    time = get_wtime() - time;
    if (info_tst != 0) {
        fprintf( stderr, "lapack::sysv_aa returned error %lld\n", (lld) info_tst );
    }
...
void test_sysv_aa( Params& params, bool run )
{
    switch (params.datatype.value()) {
        ...
        case libtest::DataType::Double:
            test_sysv_aa_work< double >( params, run );
            break;

        case libtest::DataType::SingleComplex:
            test_sysv_aa_work< std::complex<float> >( params, run );
            break;
...
void test_sysv_aa( Params& params, bool run )
{
    fprintf( stderr, "sysv_aa requires LAPACK >= 3.7\n\n" );
    exit(0);
}
