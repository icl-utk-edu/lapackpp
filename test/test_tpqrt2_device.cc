...
void test_tpqrt2_device_work( Params& params, bool run )
{
    ...
    size_t size_T = (size_t) ldt * n;  // n-by-n
    ...
    blas::device_copy_matrix( n, n, T_tst.data(), ldt, dT_tst, ldt, queue );
    ...
    int64_t info_tst = lapack::tpqrt2(
        m, n, L, dA_tst, lda, dB_tst, ldb, dT_tst, ldt, queue );
    queue.sync();
    ...
    blas::device_copy_matrix( n, n, dT_tst, ldt, T_tst.data(), ldt, queue );
    queue.sync();
    ...
    if (verbose >= 2) {
        ...
        printf( "T = "     ); print_matrix( n,   n, &T_tst[0],   ldt );
        printf( "\n" );
    }
    ...
    if (params.check() == 'y') {
        ...
        info_tst = lapack::tpmqrt(
            lapack::Side::Left, lapack::Op::NoTrans, m, n, n, L, n,
    ...
    if (params.ref() == 'y') {
        ...
        int64_t info_ref = LAPACKE_tpqrt2(
...
void test_tpqrt2_device( Params& params, bool run )
{
    switch (params.datatype()) {
        case testsweeper::DataType::Single:
            test_tpqrt2_device_work< float >( params, run );
            break;

        case testsweeper::DataType::Double:
            test_tpqrt2_device_work< double >( params, run );
            break;

        case testsweeper::DataType::SingleComplex:
            test_tpqrt2_device_work< std::complex<float> >( params, run );
            break;

        case testsweeper::DataType::DoubleComplex:
            test_tpqrt2_device_work< std::complex<double> >( params, run );
            break;
